...
//    @JsonFormat(pattern = "yyyy-MM-dd",timezone="GMT+8")
    ...
    @TableField(exist = false)
    private Integer wsId;//一定不能删除，影响机台传递参数信息
    ...
    @ApiModelProperty(value = "ERP的主施工单UUID")
    private String erpWbid;
    @ApiModelProperty(value = "产品id")
    private String pdId;

    private static WorkbatchOrdlink instance;

    public WorkbatchOrdlink() {

    }


    public static WorkbatchOrdlink getInstance() {
        if (instance == null) {
            synchronized (WorkbatchOrdlink.class) {
                if (instance == null) {
                    instance = new WorkbatchOrdlink();
                }
            }
        }
        return instance;
    ...
    public static WorkbatchOrdlink getInstanceProduction() {
        getInstance().setStatus(OrderStatusConstant.STATUS_PRODUCTION);
        getInstance().setRunStatus(OrderStatusConstant.RUN_STATUS_PRODUCTION);
        return instance;
    }


    public static WorkbatchOrdlink getInstanceWaiting() {
        getInstance().setStatus(OrderStatusConstant.STATUS_WAITING);
        getInstance().setRunStatus(OrderStatusConstant.RUN_STATUS_WAITING);
        return instance;
    }

    public static WorkbatchOrdlink getInstanceNOREPORT() {
        getInstance().setStatus(OrderStatusConstant.STATUS_PRODUCTION);
        getInstance().setRunStatus(OrderStatusConstant.RUN_STATUS_NOREPORT);
        return instance;
    }

...
import com.yb.system.dept.wrapper.DeptWrapper;
...
public class SaDeptController extends BladeController {
    ...
    public R<List<INode>> list(@ApiIgnore @RequestParam Map<String, Object> dept, SaUser bladeUser) {
    ...
    public R<List<SaDeptVO>> tree(String tenantId, SaUser bladeUser) {
        bladeUser = SaSecureUtil.getUser();
    ...
    public R submit(@Valid @RequestBody BaseDeptinfo dept, SaUser user) {
    ...
    public R remove(@ApiParam(value = "主键集合", required = true) @RequestParam String ids) {
        return R.status(deptService.updateIsdelete(Func.toIntList(ids)));
    }
    ...
    public R<IPage<BaseDeptinfo>> lists(Query query,@ApiIgnore @RequestParam() Map<String, Object> baseDeptinfo) {
        ...
        if(!baseDeptinfo.isEmpty()){
            for (Map.Entry<String,Object> entry: baseDeptinfo.entrySet()){
                if(entry.getKey().equals("classify")){
                    baseDeptinfo1.setClassify(Integer.parseInt((String)entry.getValue()));
                }
    ...
    public R<List<BaseDeptinfo>> listdepts(){
        SaUser Sauser = SaSecureUtil.getUser();
    ...
    @ApiOperationSupport(order = 2)
    @ApiOperation(value = "获取生产车间列表", notes = "")
    public R<List<BaseDeptinfo>> produceList(){
        Map<String,Object> map = new HashMap<>();
        map.put("is_deleted",0);
        map.put("classify",2);
        return R.data(deptService.listByMap(map));

...
//    @PreAuth(RoleConstant.HAS_ROLE_ADMIN)
     ...
     * 印聊菜单数据
     */
    @GetMapping("/getAppRoutes")
    @ApiOperationSupport(order = 5)
    @ApiOperation(value = "印聊菜单数据", notes = "印聊菜单数据")
    public R<List<MenuVO>> getAppRoutes(Integer userId) {
        SaUser user = saUserMapper.selectById(userId);
        if (user == null) {
            log.info("获取印聊菜单失败,登录用户不存在:userId:{}", userId);
            throw new CommonException(HttpStatus.NOT_FOUND.value(), "获取印聊菜单失败,登录用户不存在");
        }
        List<MenuVO> list = menuService.getAppRoutes(user.getRoleId());
        return R.data(list);
    }
}
